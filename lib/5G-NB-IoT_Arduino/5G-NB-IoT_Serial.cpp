...
_5G_NB_IoT_Serial::_5G_NB_IoT_Serial() : _atserial(Serial), _dserial(Serial)
{
}

_5G_NB_IoT_Serial::~_5G_NB_IoT_Serial()
{
    cleanBuffer();
}

_5G_NB_IoT_Serial::_5G_NB_IoT_Serial(Stream &atserial, Stream &dserial) : _atserial(atserial), _dserial(dserial)
{
...
void _5G_NB_IoT_Serial::AT_bypass()
{
...
template <class T>
bool _5G_NB_IoT_Serial::AutoATSerialBand(T &_atserial)
{
...
bool _5G_NB_IoT_Serial::sendDataAndCheck(const char *data_buf, const char *ok_str, const char *err_str, unsigned int timeout)
{
    delay(100);
...
bool _5G_NB_IoT_Serial::sendATcommand(const char *command)
{
    delay(100);
    while(_atserial.read()>=0);
    _atserial.write("AT");
...
unsigned int _5G_NB_IoT_Serial::readResponseByteToBuffer()
{
...
unsigned int _5G_NB_IoT_Serial::readResponseToBuffer(unsigned int timeout)
{
...
Cmd_Response_t _5G_NB_IoT_Serial::readResponseAndSearchChr(const char test_chr, unsigned int timeout)
{
    unsigned long start_time = millis();
    unsigned int recv_len = 0;
    cleanBuffer();
...
Cmd_Response_t _5G_NB_IoT_Serial::readResponseAndSearch(const char *test_str, unsigned int timeout)
{
    unsigned long start_time = millis();
    unsigned int recv_len = 0;
    cleanBuffer();
...
Cmd_Response_t _5G_NB_IoT_Serial::readResponseAndSearch(const char *test_str, const char *e_test_str, unsigned int timeout)
{
    unsigned long start_time = millis();
    unsigned int recv_len = 0;
    errorCode = -1;
...
Cmd_Response_t _5G_NB_IoT_Serial::sendAndSearchChr(const char *command, const char test_chr, unsigned int timeout)
{
    for (int i = 0; i < 3; i++){
...
Cmd_Response_t _5G_NB_IoT_Serial::sendAndSearch(const char *command, const char *test_str, unsigned int timeout)
{
    for (int i = 0; i < 3; i++){
        if(sendATcommand(command)){
...
Cmd_Response_t _5G_NB_IoT_Serial::sendAndSearch(const char *command, const char *test_str, const char *e_test_str, unsigned int timeout)
{
...
char *_5G_NB_IoT_Serial::searchStrBuffer(const char *test_str)
{
...
char *_5G_NB_IoT_Serial::searchChrBuffer(const char test_chr)
{
    int buf_len = strlen((const char *)rxBuffer);
    if (buf_len < RX_BUFFER_LENGTH){
...
bool _5G_NB_IoT_Serial::returnErrorCode(int &s_err_code)
{
    s_err_code = -1;
    if(errorCode != -1){
...
void _5G_NB_IoT_Serial::cleanBuffer()
{
    memset(rxBuffer,'\0',RX_BUFFER_LENGTH);
    bufferHead = 0;
}

int _5G_NB_IoT_Serial::serialAvailable()
{
    unsigned int ret;
