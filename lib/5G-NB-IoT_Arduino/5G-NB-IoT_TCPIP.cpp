...
_5G_NB_IoT_TCPIP::_5G_NB_IoT_TCPIP()
{
...
_5G_NB_IoT_TCPIP::_5G_NB_IoT_TCPIP(Stream &atserial, Stream &dserial):_5G_NB_IoT_Common(atserial,dserial)
{

}

bool _5G_NB_IoT_TCPIP::SetDevAPNParameters(unsigned int pdp_index, Protocol_Type_t type, char *apn, char *usr, char *pwd, Authentication_Methods_t met)
{
    char cmd[64], buf[64];
...
Cmd_Response_t _5G_NB_IoT_TCPIP::ActivateDevAPN(unsigned int pdp_index)
{
    char cmd[16],buf[8];
...
bool _5G_NB_IoT_TCPIP::DeactivateDevAPN(unsigned int pdp_index)
{
    char cmd[16],buf[8];
...
bool _5G_NB_IoT_TCPIP::GetDevAPNIPAddress(unsigned int pdp_index, char *ip)
{
    char cmd[16],buf[8];
...
bool _5G_NB_IoT_TCPIP::InitAPN(unsigned int pdp_index, char *apn, char *usr, char *pwd, char *err_code)
{
...
bool _5G_NB_IoT_TCPIP::OpenSocketService(unsigned int pdp_index, unsigned int socket_index, Socket_Type_t socket, char *ip, unsigned int port, unsigned int local_port, Access_Mode_t mode)
{
...
bool _5G_NB_IoT_TCPIP::CloseSocketService(unsigned int socket_index)
{
    char cmd[16],buf[8];
...
bool _5G_NB_IoT_TCPIP::SocketSendData(unsigned int socket_index, Socket_Type_t socket, char *data_buf, char *ip, unsigned int port)
{
    char cmd[64],buf[64];
...
bool _5G_NB_IoT_TCPIP::SocketRecvData(unsigned int socket_index, unsigned int recv_len, Socket_Type_t socket, char *recv_buf)
{
    char cmd[16],buf[16];
...
bool _5G_NB_IoT_TCPIP::SocketSendHEXData(unsigned int socket_index, char *hex_buf)
{
...
bool _5G_NB_IoT_TCPIP::SwitchAccessModes(unsigned int socket_index, Access_Mode_t mode)
{
    char cmd[16],buf[16];
...
bool _5G_NB_IoT_TCPIP::DevPingFunction(unsigned int socket_index, char *host)
{
    char cmd[128],buf[64];
...
bool _5G_NB_IoT_TCPIP::DevNTPFunction(unsigned int socket_index, char *ntp_ip, unsigned int port, char *time)
{
    char cmd[64],buf[64];
...
bool _5G_NB_IoT_TCPIP::ConfigDNSServer(unsigned int socket_index, char *primary_addr, char *secondary_addr)
{
    char cmd[128],buf[128];
...
bool _5G_NB_IoT_TCPIP::DevDNSFunction(unsigned int socket_index, char *domain_name, char *ip)
{
    char cmd[128],buf[128];
...
bool _5G_NB_IoT_TCPIP::QueryLastErrorCode(char *err_code)
{
    char cmd[16];
...
Socket_Event_t _5G_NB_IoT_TCPIP::WaitCheckSocketEvent(char *event, unsigned int timeout)
{
